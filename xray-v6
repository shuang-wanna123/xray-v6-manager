...
generate_xray_config() {
    ...
    [ "$HAS_IPV4" = "true" ] && {
        outbounds+='{"tag":"IPv4-out","protocol":"freedom","sendThrough":"'"$VPS_IP"'","settings":{"domainStrategy":"UseIPv4"}}'
        first_outbound=false
    }

    [ "$HAS_IPV6" = "true" ] && {
        [ "$first_outbound" = "false" ] && outbounds+=","
        outbounds+='{"tag":"IPv6-out","protocol":"freedom","sendThrough":"'"$IPV6_ADDR"'","settings":{"domainStrategy":"UseIPv6"}}'
        first_outbound=false
    }
...
generate_info() {
    ...
    local ep_v4=$(url_encode_path "$VLESS_V4_PATH")
    local ep_v6=$(url_encode_path "$VLESS_V6_PATH")
    local ep_p6=$(url_encode_path "$VLESS_P6_PATH")
    local node_num=1

    {
        ...
        echo "  优选地址: $CDN_HOST  |  启用节点: ${enabled}/5  |  WebRTC拦截: ✅"
        echo "═══════════════════════════════════════════════════════════════════════════════"

        if [ "$SOCKS5_P6_ENABLED" = "true" ]; then
            local cip=$(format_host "${VPS_IP:-$IPV6_ADDR}")
        ...
        if [ "$SOCKS5_V6_ENABLED" = "true" ] && [ "$HAS_IPV6" = "true" ]; then
            local cip=$(format_host "${VPS_IP:-$IPV6_ADDR}")
            echo -e "\n  节点 ${node_num}: SOCKS5-强制IPv6（端口 ${SOCKS5_V6_PORT}）出站: 仅IPv6 → ${IPV6_ADDR}"
...
load_params() {
    [ -f "$PARAMS_FILE" ] && source "$PARAMS_FILE" && return 0
    echo -e "${RED}参数文件不存在${NC}"
    return 1
}
...
get_random_port() {
    for _ in {1..100}; do
        local p=$((RANDOM % ($2 - $1 + 1) + $1))
        check_port_available "$p" && echo "$p" && return 0
    done
    echo $((RANDOM % ($2 - $1 + 1) + $1))
}
...
regenerate_all() {
    ...
    [ "$SOCKS5_P6_ENABLED" = "true" ] && {
        [ "$first" = "false" ] && inbounds+=","
        inbounds+='{"tag":"socks-p6-in","listen":"::","port":'$SOCKS5_P6_PORT',"protocol":"socks","settings":{"auth":"password","accounts":[{"user":"'$SOCKS5_P6_USER'","pass":"'$SOCKS5_P6_PASS'"}],"udp":true}}'
        first=false
    }

    [ "$SOCKS5_V6_ENABLED" = "true" ] && [ "$HAS_IPV6" = "true" ] && {
        [ "$first" = "false" ] && inbounds+=","
        inbounds+='{"tag":"socks-v6-in","listen":"::","port":'$SOCKS5_V6_PORT',"protocol":"socks","settings":{"auth":"password","accounts":[{"user":"'$SOCKS5_V6_USER'","pass":"'$SOCKS5_V6_PASS'"}],"udp":true}}'
        first=false
    }

    [ "$VLESS_V4_ENABLED" = "true" ] && [ "$HAS_IPV4" = "true" ] && {
        [ "$first" = "false" ] && inbounds+=","
    ...
    [ "$VLESS_V6_ENABLED" = "true" ] && [ "$HAS_IPV6" = "true" ] && {
        [ "$first" = "false" ] && inbounds+=","
    ...
    [ "$VLESS_P6_ENABLED" = "true" ] && {
        [ "$first" = "false" ] && inbounds+=","
        inbounds+='{"tag":"vless-p6-in","listen":"::","port":'$VLESS_P6_PORT',"protocol":"vless","settings":{"clients":[{"id":"'$UUID'"}],"decryption":"none"},"streamSettings":{"network":"ws","security":"tls","tlsSettings":{"certificates":[{"certificateFile":"'$DIR'/cert/fullchain.crt","keyFile":"'$DIR'/cert/private.key"}]},"wsSettings":{"path":"'$VLESS_P6_PATH'"}}}'
        first=false
    }
    ...
    "$DIR/bin/xray" run -test -c "$DIR/config/config.json" >/dev/null 2>&1 || {
...

    {
        ...
        if [ "$SOCKS5_P6_ENABLED" = "true" ]; then
            local cip=$(format_host "${VPS_IP:-$IPV6_ADDR}")
            echo -e "\n  节点 ${node_num}: SOCKS5-优先IPv6（端口 ${SOCKS5_P6_PORT}）"
            echo "  socks5://${SOCKS5_P6_USER}:${SOCKS5_P6_PASS}@${cip}:${SOCKS5_P6_PORT}#SOCKS5-${node_num}"
            ((node_num++))
        fi
        ...
        [ "$VLESS_P6_ENABLED" = "true" ] && {
            echo -e "\n  节点 ${node_num}: VLESS-优先IPv6（端口 ${VLESS_P6_PORT}）"
            echo "  vless://${UUID}@${CDN_HOST}:${VLESS_P6_PORT}?encryption=none&security=tls&sni=${DOMAIN}&type=ws&host=${DOMAIN}&path=${ep_p6}#IPv4%26IPv6-CDN-${node_num}"
            ((node_num++))
        }
...
show_menu() {
    ...
    echo -e "  节点: ${YELLOW}${enabled}/5${NC}"
    echo -e "  UUID: ${YELLOW}${UUID:0:8}...${NC}"
    echo -e "  WebRTC: ${GREEN}✅${NC}"
...
show_info() {
    clear
    [ -f "$DIR/info.txt" ] && cat "$DIR/info.txt" || echo -e "${RED}信息文件不存在${NC}"
    echo -e "\n${YELLOW}按回车返回...${NC}"
    read -r
}
...
start_service() {
    ...
    sleep 2
    systemctl is-active --quiet $SERVICE && echo -e "${GREEN}成功${NC}" || {
        echo -e "${RED}失败${NC}"
...
show_node_status() {
    ...
    [ "$VLESS_V4_ENABLED" = "true" ] && [ "$HAS_IPV4" = "true" ] && s3="✅"
    [ "$VLESS_V6_ENABLED" = "true" ] && [ "$HAS_IPV6" = "true" ] && s4="✅"
    [ "$VLESS_P6_ENABLED" = "true" ] && s5="✅"

    echo "  1. SOCKS5-优先IPv6  [$s1] 端口: ${SOCKS5_P6_PORT:-未设置}"
    echo "  2. SOCKS5-强制IPv6  [$s2] 端口: ${SOCKS5_V6_PORT:-未设置} $([ "$HAS_IPV6" != "true" ] && echo "(无IPv6)")"
    echo "  3. VLESS-强制IPv4   [$s3] 端口: ${VLESS_V4_PORT:-未设置} $([ "$HAS_IPV4" != "true" ] && echo "(无IPv4)")"
    echo "  4. VLESS-强制IPv6   [$s4] 端口: ${VLESS_V6_PORT:-未设置} $([ "$HAS_IPV6" != "true" ] && echo "(无IPv6)")"
    echo "  5. VLESS-优先IPv6   [$s5] 端口: ${VLESS_P6_PORT:-未设置}"
    echo ""
...
toggle_node() {
    load_params || return
    echo ""
    echo -e "${CYAN}─── 启用/禁用节点 ───${NC}"
    echo "  1. SOCKS5-优先IPv6  [$([ "$SOCKS5_P6_ENABLED" = "true" ] && echo "✅" || echo "❌")]"
    echo "  2. SOCKS5-强制IPv6  [$([ "$SOCKS5_V6_ENABLED" = "true" ] && [ "$HAS_IPV6" = "true" ] && echo "✅" || echo "❌")]$([ "$HAS_IPV6" != "true" ] && echo " (无IPv6)")"
    echo "  3. VLESS-强制IPv4   [$([ "$VLESS_V4_ENABLED" = "true" ] && [ "$HAS_IPV4" = "true" ] && echo "✅" || echo "❌")]$([ "$HAS_IPV4" != "true" ] && echo " (无IPv4)")"
    echo "  4. VLESS-强制IPv6   [$([ "$VLESS_V6_ENABLED" = "true" ] && [ "$HAS_IPV6" = "true" ] && echo "✅" || echo "❌")]$([ "$HAS_IPV6" != "true" ] && echo " (无IPv6)")"
    echo "  5. VLESS-优先IPv6   [$([ "$VLESS_P6_ENABLED" = "true" ] && echo "✅" || echo "❌")]"
    echo "  0. 返回"
    echo ""
    echo -n "选择: "
    read -r c
    ...
    case $c in
        ...
        1)
            if [ "$SOCKS5_P6_ENABLED" = "true" ]; then
                SOCKS5_P6_ENABLED="false"
            else
                [ -z "$SOCKS5_P6_PORT" ] && SOCKS5_P6_PORT=$(get_random_port 20000 30000)
                [ -z "$SOCKS5_P6_USER" ] && SOCKS5_P6_USER=$(random_string 10)
                [ -z "$SOCKS5_P6_PASS" ] && SOCKS5_P6_PASS=$(random_password 12)
                SOCKS5_P6_ENABLED="true"
            fi
            ;;
        2)
            [ "$HAS_IPV6" != "true" ] && { echo -e "${RED}VPS 无 IPv6，无法启用${NC}"; sleep 2; return; }
            if [ "$SOCKS5_V6_ENABLED" = "true" ]; then
                SOCKS5_V6_ENABLED="false"
            else
                [ -z "$SOCKS5_V6_PORT" ] && SOCKS5_V6_PORT=$(get_random_port 30001 40000)
            ...
            ;;
        3)
            [ "$HAS_IPV4" != "true" ] && { echo -e "${RED}VPS 无 IPv4，无法启用${NC}"; sleep 2; return; }
            if [ "$VLESS_V4_ENABLED" = "true" ]; then
                VLESS_V4_ENABLED="false"
            else
                [ -z "$VLESS_V4_PORT" ] && VLESS_V4_PORT="2053"
                [ -z "$VLESS_V4_PATH" ] && VLESS_V4_PATH="/$(random_string 8)"
        ...
        4)
            ...
            if [ "$VLESS_V6_ENABLED" = "true" ]; then
                ...
                [ -z "$VLESS_V6_PORT" ] && VLESS_V6_PORT="2083"
                [ -z "$VLESS_V6_PATH" ] && VLESS_V6_PATH="/$(random_string 8)"
                VLESS_V6_ENABLED="true"
            fi
            ;;
        5)
            if [ "$VLESS_P6_ENABLED" = "true" ]; then
                VLESS_P6_ENABLED="false"
            else
                [ -z "$VLESS_P6_PORT" ] && VLESS_P6_PORT="2087"
                [ -z "$VLESS_P6_PATH" ] && VLESS_P6_PATH="/$(random_string 8)"
                VLESS_P6_ENABLED="true"
            fi
            ;;
...
modify_node() {
    ...
    echo ""
    echo -e "${CYAN}─── 修改节点 ───${NC}"
    echo "  1. SOCKS5-优先IPv6  端口: ${SOCKS5_P6_PORT:-未设置}"
    echo "  2. SOCKS5-强制IPv6  端口: ${SOCKS5_V6_PORT:-未设置}"
    echo "  3. VLESS-强制IPv4   端口: ${VLESS_V4_PORT:-未设置}  路径: ${VLESS_V4_PATH:-未设置}"
    echo "  4. VLESS-强制IPv6   端口: ${VLESS_V6_PORT:-未设置}  路径: ${VLESS_V6_PATH:-未设置}"
    echo "  5. VLESS-优先IPv6   端口: ${VLESS_P6_PORT:-未设置}  路径: ${VLESS_P6_PATH:-未设置}"
    echo "  0. 返回"
    echo ""
    echo -n "选择: "
    read -r c
    ...
    case $c in
        0) return ;;
        1)
            echo -n "新端口(当前 $SOCKS5_P6_PORT): "; read -r p
            [ -n "$p" ] && SOCKS5_P6_PORT="$p"
            echo -n "新用户名(当前 $SOCKS5_P6_USER): "; read -r u
            [ -n "$u" ] && SOCKS5_P6_USER="$u"
            echo -n "新密码(当前 $SOCKS5_P6_PASS): "; read -r w
            [ -n "$w" ] && SOCKS5_P6_PASS="$w"
            ;;
        2)
            echo -n "新端口(当前 $SOCKS5_V6_PORT): "; read -r p
            [ -n "$p" ] && SOCKS5_V6_PORT="$p"
            echo -n "新用户名(当前 $SOCKS5_V6_USER): "; read -r u
            [ -n "$u" ] && SOCKS5_V6_USER="$u"
            echo -n "新密码(当前 $SOCKS5_V6_PASS): "; read -r w
            [ -n "$w" ] && SOCKS5_V6_PASS="$w"
            ;;
        3)
            echo -n "新端口(当前 $VLESS_V4_PORT): "; read -r p
            [ -n "$p" ] && VLESS_V4_PORT="$p"
            echo -n "新路径(当前 $VLESS_V4_PATH): "; read -r t
            [ -n "$t" ] && VLESS_V4_PATH="$t"
            ;;
        4)
            echo -n "新端口(当前 $VLESS_V6_PORT): "; read -r p
            [ -n "$p" ] && VLESS_V6_PORT="$p"
            echo -n "新路径(当前 $VLESS_V6_PATH): "; read -r t
            [ -n "$t" ] && VLESS_V6_PATH="$t"
            ;;
        5)
            echo -n "新端口(当前 $VLESS_P6_PORT): "; read -r p
            [ -n "$p" ] && VLESS_P6_PORT="$p"
            echo -n "新路径(当前 $VLESS_P6_PATH): "; read -r t
            [ -n "$t" ] && VLESS_P6_PATH="$t"
            ;;
        *) echo -e "${RED}无效选项${NC}"; sleep 1; return ;;
    esac
...
add_node() {
    ...
    echo ""
    echo -e "${CYAN}─── 新增节点 ───${NC}"
    echo "  1. SOCKS5-优先IPv6  $([ "$SOCKS5_P6_ENABLED" = "true" ] && echo "(已启用)" || echo "(未启用)")"
    echo "  2. SOCKS5-强制IPv6  $([ "$SOCKS5_V6_ENABLED" = "true" ] && echo "(已启用)" || echo "(未启用)")$([ "$HAS_IPV6" != "true" ] && echo " [无IPv6]")"
    echo "  3. VLESS-强制IPv4   $([ "$VLESS_V4_ENABLED" = "true" ] && echo "(已启用)" || echo "(未启用)")$([ "$HAS_IPV4" != "true" ] && echo " [无IPv4]")"
    echo "  4. VLESS-强制IPv6   $([ "$VLESS_V6_ENABLED" = "true" ] && echo "(已启用)" || echo "(未启用)")$([ "$HAS_IPV6" != "true" ] && echo " [无IPv6]")"
    echo "  5. VLESS-优先IPv6   $([ "$VLESS_P6_ENABLED" = "true" ] && echo "(已启用)" || echo "(未启用)")"
    echo "  0. 返回"
    echo ""
    echo -n "选择: "
    read -r c
    ...
    case $c in
        0) return ;;
        1)
            [ "$SOCKS5_P6_ENABLED" = "true" ] && { echo -e "${YELLOW}已启用，请使用修改节点${NC}"; sleep 2; return; }
            echo -n "端口(回车随机): "; read -r p
            [ -z "$p" ] && p=$(get_random_port 20000 30000)
            SOCKS5_P6_PORT="$p"
            echo -n "用户名(回车随机): "; read -r u
            [ -z "$u" ] && u=$(random_string 10)
            SOCKS5_P6_USER="$u"
            echo -n "密码(回车随机): "; read -r w
            [ -z "$w" ] && w=$(random_password 12)
            SOCKS5_P6_PASS="$w"
            SOCKS5_P6_ENABLED="true"
            ;;
        2)
            [ "$HAS_IPV6" != "true" ] && { echo -e "${RED}VPS 无 IPv6，无法新增${NC}"; sleep 2; return; }
            [ "$SOCKS5_V6_ENABLED" = "true" ] && { echo -e "${YELLOW}已启用，请使用修改节点${NC}"; sleep 2; return; }
            echo -n "端口(回车随机): "; read -r p
            [ -z "$p" ] && p=$(get_random_port 30001 40000)
            SOCKS5_V6_PORT="$p"
            echo -n "用户名(回车随机): "; read -r u
            [ -z "$u" ] && u=$(random_string 10)
            SOCKS5_V6_USER="$u"
            ...
            ;;
        3)
            [ "$HAS_IPV4" != "true" ] && { echo -e "${RED}VPS 无 IPv4，无法新增${NC}"; sleep 2; return; }
            [ "$VLESS_V4_ENABLED" = "true" ] && { echo -e "${YELLOW}已启用，请使用修改节点${NC}"; sleep 2; return; }
            echo -n "端口(默认 2053): "; read -r p
            [ -z "$p" ] && p="2053"
            VLESS_V4_PORT="$p"
            echo -n "路径(回车随机): "; read -r t
            [ -z "$t" ] && t="/$(random_string 8)"
            VLESS_V4_PATH="$t"
            VLESS_V4_ENABLED="true"
            ;;
        4)
            [ "$HAS_IPV6" != "true" ] && { echo -e "${RED}VPS 无 IPv6，无法新增${NC}"; sleep 2; return; }
            [ "$VLESS_V6_ENABLED" = "true" ] && { echo -e "${YELLOW}已启用，请使用修改节点${NC}"; sleep 2; return; }
            echo -n "端口(默认 2083): "; read -r p
            [ -z "$p" ] && p="2083"
            VLESS_V6_PORT="$p"
            echo -n "路径(回车随机): "; read -r t
            [ -z "$t" ] && t="/$(random_string 8)"
            ...
            ;;
        5)
            [ "$VLESS_P6_ENABLED" = "true" ] && { echo -e "${YELLOW}已启用，请使用修改节点${NC}"; sleep 2; return; }
            echo -n "端口(默认 2087): "; read -r p
            [ -z "$p" ] && p="2087"
            VLESS_P6_PORT="$p"
            echo -n "路径(回车随机): "; read -r t
            [ -z "$t" ] && t="/$(random_string 8)"
...
delete_node() {
    ...
    echo -e "${CYAN}─── 删除节点 ───${NC}"
    echo "  1. SOCKS5-优先IPv6  [$([ "$SOCKS5_P6_ENABLED" = "true" ] && echo "✅" || echo "❌")]"
    echo "  2. SOCKS5-强制IPv6  [$([ "$SOCKS5_V6_ENABLED" = "true" ] && echo "✅" || echo "❌")]"
    echo "  3. VLESS-强制IPv4   [$([ "$VLESS_V4_ENABLED" = "true" ] && echo "✅" || echo "❌")]"
    echo "  4. VLESS-强制IPv6   [$([ "$VLESS_V6_ENABLED" = "true" ] && echo "✅" || echo "❌")]"
    echo "  5. VLESS-优先IPv6   [$([ "$VLESS_P6_ENABLED" = "true" ] && echo "✅" || echo "❌")]"
    echo "  0. 返回"
    echo ""
    echo -n "选择删除: "
    ...
    case $c in
        0) return ;;
        1) SOCKS5_P6_ENABLED="false"; SOCKS5_P6_PORT=""; SOCKS5_P6_USER=""; SOCKS5_P6_PASS="" ;;
        2) SOCKS5_V6_ENABLED="false"; SOCKS5_V6_PORT=""; SOCKS5_V6_USER=""; SOCKS5_V6_PASS="" ;;
        3) VLESS_V4_ENABLED="false"; VLESS_V4_PORT=""; VLESS_V4_PATH="" ;;
        4) VLESS_V6_ENABLED="false"; VLESS_V6_PORT=""; VLESS_V6_PATH="" ;;
        5) VLESS_P6_ENABLED="false"; VLESS_P6_PORT=""; VLESS_P6_PATH="" ;;
        *) echo -e "${RED}无效选项${NC}"; sleep 1; return ;;
    esac
...
modify_cdn() {
    load_params || return
    echo ""
    echo -e "当前 CDN 优选地址: ${YELLOW}$CDN_HOST${NC}"
    echo -n "新 CDN 优选地址: "
    read -r c
    [ -n "$c" ] && CDN_HOST="$c"
    save_params
    echo -e "${GREEN}CDN 优选地址已更新: $CDN_HOST${NC}"
    regenerate_all
    sleep 2
}

edit_config() {
    command -v nano &>/dev/null && nano "$DIR/config/config.json" || vi "$DIR/config/config.json"
    echo -n -e "${YELLOW}重启服务？(y/n): ${NC}"
    read -r r
    [ "$r" = "y" ] || [ "$r" = "Y" ] && restart_service
}
...
update_xray() {
    clear
    local cur=$("$DIR/bin/xray" version 2>/dev/null | head -1 | awk '{print $2}')
    local lat=$(curl -sL --max-time 15 "https://api.github.com/repos/XTLS/Xray-core/releases/latest" | grep -o '"tag_name": *"[^"]*"' | head -1 | cut -d'"' -f4)

    echo -e "当前版本: ${YELLOW}${cur:-未知}${NC}"
    echo -e "最新版本: ${GREEN}${lat:-获取失败}${NC}"

    [ -z "$lat" ] && { echo -e "${RED}无法获取最新版本${NC}"; sleep 3; return; }
    [ "$cur" = "$lat" ] && { echo -e "${GREEN}已是最新版本${NC}"; sleep 3; return; }

    echo -n -e "${YELLOW}更新？(y/n): ${NC}"
    read -r u
    [ "$u" != "y" ] && [ "$u" != "Y" ] && return
    ...
    case "$arch" in
        ...
        *) echo -e "${RED}不支持架构: $arch${NC}"; sleep 3; return ;;
    esac

    cd "$DIR" || return
    if wget -q --show-progress -O xray.zip "https://github.com/XTLS/Xray-core/releases/download/${lat}/Xray-linux-${arch}.zip"; then
...
uninstall() {
    clear
    echo -e "${RED}警告：此操作会删除所有配置和服务！${NC}"
    echo -n "输入 yes 确认卸载: "
    read -r c
...
print_banner() {
    ...
    echo "║  • SOCKS5-优先IPv6 / SOCKS5-强制IPv6                         ║"
    echo "║  • VLESS-强制IPv4 / VLESS-强制IPv6 / VLESS-优先IPv6           ║"
...
check_root() {
    [ "$(id -u)" != "0" ] && { print_error "请使用 root 用户运行"; exit 1; }
    print_success "Root 权限通过"
}
...
get_user_input() {
    echo ""
    echo -e "${CYAN}╔═══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║                    快速配置向导                              ║${NC}"
    echo -e "${CYAN}║              其余配置将使用默认值自动生成                     ║${NC}"
    echo -e "${CYAN}╚═══════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    ...
    SOCKS5_P6_ENABLED="true"
    SOCKS5_P6_PORT=$(get_random_port 20000 30000)
    SOCKS5_P6_USER=$(random_string 10)
    SOCKS5_P6_PASS=$(random_password 12)

    if [ "$HAS_IPV6" = "true" ]; then
        SOCKS5_V6_ENABLED="true"
        SOCKS5_V6_PORT=$(get_random_port 30001 40000)
    ...
    VLESS_P6_ENABLED="true"
    VLESS_P6_PORT="2087"
    VLESS_P6_PATH="/$(random_string 8)"

    echo ""
    echo -e "${CYAN}═══════════════════════════════════════════════════════════════${NC}"
    echo -e "  域名: ${YELLOW}$DOMAIN${NC}"
    echo -e "  UUID: ${YELLOW}${UUID:0:8}...${NC}"
    echo -e "  CDN:  ${YELLOW}$CDN_HOST${NC}"
    echo -e "  IPv4: ${YELLOW}${VPS_IP:-无}${NC}"
    echo -e "  IPv6: ${YELLOW}${IPV6_ADDR:-无}${NC}"
    echo ""
    echo "  将启用节点:"
    [ "$SOCKS5_P6_ENABLED" = "true" ] && echo -e "    ${GREEN}✓${NC} SOCKS5-优先IPv6  :$SOCKS5_P6_PORT"
    [ "$SOCKS5_V6_ENABLED" = "true" ] && echo -e "    ${GREEN}✓${NC} SOCKS5-强制IPv6  :$SOCKS5_V6_PORT"
    [ "$VLESS_V4_ENABLED" = "true" ] && echo -e "    ${GREEN}✓${NC} VLESS-强制IPv4   :$VLESS_V4_PORT"
    [ "$VLESS_V6_ENABLED" = "true" ] && echo -e "    ${GREEN}✓${NC} VLESS-强制IPv6   :$VLESS_V6_PORT"
    [ "$VLESS_P6_ENABLED" = "true" ] && echo -e "    ${GREEN}✓${NC} VLESS-优先IPv6   :$VLESS_P6_PORT"
    echo -e "${CYAN}═══════════════════════════════════════════════════════════════${NC}"
    echo ""
...
install_deps() {
    ...
    if command -v apt-get &>/dev/null; then
        ...
        apt-get install -y curl wget unzip socat cron openssl psmisc >/dev/null 2>&1
    elif command -v yum &>/dev/null; then
        yum install -y curl wget unzip socat cronie openssl psmisc >/dev/null 2>&1
    fi
...
cleanup_old() {
    ...
    if [ -f "$INSTALL_DIR/cert/fullchain.crt" ] && [ -f "$INSTALL_DIR/cert/private.key" ]; then
        bak="/tmp/xray_cert_$$"
        mkdir -p "$bak"
        cp "$INSTALL_DIR/cert/"* "$bak/" 2>/dev/null
        print_info "已备份证书"
    fi
    ...
    if [ -n "$bak" ] && [ -d "$bak" ]; then
        ...
        cp "$bak/"* "$INSTALL_DIR/cert/" 2>/dev/null
        rm -rf "$bak"
        print_info "已恢复证书"
    fi
...
download_xray() {
    ...
    local lat
    lat=$(curl -sL --max-time 15 "https://api.github.com/repos/XTLS/Xray-core/releases/latest" | grep -o '"tag_name": *"[^"]*"' | head -1 | cut -d'"' -f4)

    if [ -z "$lat" ] || [[ ! "$lat" =~ ^v[0-9] ]]; then
        lat="v24.12.18"
        print_warning "无法获取最新版本，使用备用版本: $lat"
    else
        print_info "版本: $lat"
    fi

    wget -q --show-progress -O xray.zip "https://github.com/XTLS/Xray-core/releases/download/${lat}/Xray-linux-${XRAY_ARCH}.zip" || {
        print_error "下载失败"
        exit 1
    }
...
install_cert() {
    ...
    if [ -f "$INSTALL_DIR/cert/fullchain.crt" ] && [ -f "$INSTALL_DIR/cert/private.key" ]; then
        if openssl x509 -checkhost "$DOMAIN" -noout -in "$INSTALL_DIR/cert/fullchain.crt" >/dev/null 2>&1 && \
           openssl x509 -checkend 86400 -noout -in "$INSTALL_DIR/cert/fullchain.crt" >/dev/null 2>&1; then
            print_success "使用已有证书 ($DOMAIN)"
            return 0
        else
            print_warning "证书不匹配或已过期，重新申请当前域名证书: $DOMAIN"
    ...
    print_warning "请确保域名 $DOMAIN 已解析到此 VPS，且申请证书时 CDN 代理已关闭"
    ...
    [ ! -f "$INSTALL_DIR/cert/fullchain.crt" ] && {
        print_error "证书安装失败"
        exit 1
    }
...
show_result() {
    clear
    [ -f "$INSTALL_DIR/info.txt" ] && cat "$INSTALL_DIR/info.txt"
